 ...
 */
public class UpdateAccessPolicy extends HttpServlet {
	...
	session.setAttribute("updation", null);
	...
	if(dbm.updateAccessPolicy(listAccessExpressions,listAccessValues,fileId)){
		session.setAttribute("msg","Updation of access policy successful");
		response.sendRedirect("MyFiles.jsp");
	}
	else{
		session.setAttribute("msg","Updation of access policy failed");
		response.sendRedirect("MyFiles.jsp");
	}
}
